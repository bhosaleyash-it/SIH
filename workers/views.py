...
def booking_action(request, pk, action):
    ...
    if action == "accept" and booking.status == Booking.Status.PENDING:
        ...
        payment, created = Payment.objects.get_or_create(
            booking=booking,
            defaults={"amount": booking.price, "method": Payment.Method.CASH},
        )
        if created:
        ...
        messages.success(request, _("Job marked complete. Payment is now ready for settlement."))
    elif action == "cash_received" and booking.status == Booking.Status.COMPLETED:
        payment = get_object_or_404(Payment, booking=booking, method=Payment.Method.CASH)
        if payment.status != Payment.Status.SUCCESS:
            payment.worker_confirmed_at = now
            payment.save(update_fields=["worker_confirmed_at"])
            messages.success(request, _("Cash receipt sent to the admin for approval."))
    return redirect("workers:bookings_list")
